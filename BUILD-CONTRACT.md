...
- Mode events use `{t:'ev',kind,at,...fields}`. Their kinds and supplemental
  ...
  `bomb_defuse` (`id,site`), `bomb_explode` (`site,x,y,z`),
  `interaction_start`/`interaction_cancel` (`id,action,site`), and the TTT
  `trap` (`id,name,effect,by:'traitor',x,y,z,button:[x,y,z]`) where `id` is
  the trap id, `x,y,z` the effect origin and `button` the standing point in
  front of the pressed plate; a `trap` event never carries a player id.
...
- `new Input(canvas)` then `bind(onLockChange)`; poll `getKeys()` for movement
  ...
  interact; `B` toggles the buy menu; `1–9`/`0`/wheel select weapons.
...
- **Waterworld (`waterworld`):** a block-for-block replica of
  ...
  in `public/js/engine/waterworld-details.js`. The two flumes are rides:
  `meta.slides` carries each flume's centreline path (the original
  `trigger_push` chain) and speed, and `shared/slide-rules.js`
  (`SLIDE_RULES`, `slideContact`, `stepSlideRide`) puts a body that enters a
  mouth on rails through the carved tube bore to the splash lane, prone,
  without collision or jumps and with a little steering, identically in
  prediction and on the authority (no wire change: position and `proneT`
  already travel). Free-for-all and team spawns are generated by the compiler
  (`meta.spawns.fun`, `tdm`), Trouble in Terrorist Town keeps the 66 original
  entities in `meta.spawns.ttt`, which `FunPolicy` prefers for its mode when a
...
- Trap definitions are shared data: `shared/world/traps.js` exports
  `MAP_TRAPS`, `trapsFor(mapId)`, `TRAP_RULES` (`useRange` 1.6, `botApproachRange`
  28, `weaponKey:'trap'`), `TRAP_EFFECT_KINDS`, `trapButtonPoint`,
  `trapWallCell`, `trapEffectContains`, `trapRegionBounds` and `isTrapRequest`.
  `getMapMeta(id).traps` is the same list (deep-frozen; `[]` for maps without
  traps). Each trap is `{id,name,detail,button:{x,y,z,face},uses?,cooldownMs?,
  effect:{kind,...}}` with `kind` one of `explosion|lava|flood|door_lock|
  collapse|electrify|gas|fire`.
- Client → server: a trap press is either the raw `keys.interact` rising edge
  while a living traitor stands within `useRange` of the button in the live
...
- Server → client: a successful press emits the `trap` mode event (above),
  ...
  (and again on restore or at round end), `smokeFields` entries `smoke-trap-N`
  for gas, `fireFields` for fire, and `hit`/`die`/`kill` rows with `attacker`/
  `killer` `''` and weapon `trap` for damage volumes. Corpses of trap victims
  report `weapon:'trap'`. No message ever names the traitor who pressed.
- Private state: the recipient's own player row `ttt` carries
  `traps:[{id,name,detail,x,y,z,face,effect,state:'ready'|'cooldown'|'used',
  cooldown,uses}]` only while that recipient is a living traitor in the live
  phase and the map has traps; every other row and every innocent omits the key.
- `node tools/ttt-traps-test.mjs` (in `npm run ttt:test`) is the contract
  harness: button placement on every map with traps, gating, uses, cooldown,
  block restore, damage volumes, fields, events, private wire, bots and reset.
