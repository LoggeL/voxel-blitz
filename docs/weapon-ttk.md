...
`npm run balance:simulate` erzeugt `.artifacts/ttk/current/results.json`, `summary.md` und `rocket-blast.md` für alle vierzehn Waffen, zwölf Distanzen und fünf Szenarien. Der Lauf verändert keine Spielwerte.
...
- Minigun startet im Hauptvergleich kalt. Zusatzreihen zeigen bereits drehende Rotoren mit kalter und optimal heißer Waffe. Railgun lädt im Hauptvergleich vollständig; eine zusätzliche Suche bestimmt die früheste tödliche Teilaufladung in Server-Tick-Schritten (16,7 ms) bei perfektem Kerntreffer. IRON PICK enthält zusätzlich Rückenangriffe. Kritische Treffer im Fall (×1,5) und Rückstoß (`shared/melee.js`) bleiben außen vor: Der Schütze steht am Boden und das fixierte Ziel wird nicht bewegt; ein Krit plus ein normaler Treffer (69,6 + 46,4) töten statt drei Treffern.
