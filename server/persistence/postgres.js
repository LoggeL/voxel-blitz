...
export class PostgresStore {
  ...
  async migrate() {
    ...
    const migrations = await Promise.all(['schema.sql', 'schema-cosmetics.sql', 'schema-email-recovery.sql', 'schema-keybindings.sql'].map(async (name, index) => {
  ...
  async loadAccounts() {
    return this.transaction(async client => {
      ...
      return accounts.rows.map(row => ({ version: 1, id: row.id, username: row.username,
        ...
        ...(row.keybindings == null ? {} : { keybindings: row.keybindings }),
  ...
  async writeAccount(client, record, creating = false) {
    const values = [record.id, record.username, JSON.stringify(record.password), record.recoveryHash,
      ...
      record.keybindings ? JSON.stringify(record.keybindings) : null];
    if (creating) {
      await client.query(`INSERT INTO vb_accounts(id, username, password, recovery_hash, auth_version, created_at_ms, updated_at_ms, registration_context, email_recovery, keybindings)
      ...
      const result = await client.query(`UPDATE vb_accounts SET username=$2, password=$3, recovery_hash=$4,
        auth_version=$5, created_at_ms=$6, updated_at_ms=$7, registration_context=$8, email_recovery=$9, keybindings=$10 WHERE id=$1`, values);
