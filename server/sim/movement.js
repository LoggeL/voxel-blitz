...
export function updateTimers(p, dt) {
  ...
  if (p.reloading) {
    // Older fixtures may supply only the legacy magazine timer.
    p.reloadState ??= { staged: false, seconds: Math.max(0, p.reloadT), elapsed: 0, done: false };
    const ammo = { mag: p.mag[p.weapon], reserve: p.reserve[p.weapon] };
    advanceReload(p.reloadState, def, ammo, dt, p.infiniteMagazines);
    p.mag[p.weapon] = ammo.mag;
    p.reserve[p.weapon] = ammo.reserve;
    p.reloadT = Math.max(0, p.reloadState.seconds - p.reloadState.elapsed);
    p.reloadStage = reloadPhase(p.reloadState);
    if (p.reloadState.done) clearReload(p);
  }
}
