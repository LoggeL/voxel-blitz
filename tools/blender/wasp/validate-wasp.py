#!/usr/bin/env python3
"""Independently validate the WASP delivery by re-importing it fresh.

Usage:
    blender --background --factory-startup --python tools/blender/wasp/validate-wasp.py
    blender --background --factory-startup --python tools/blender/wasp/validate-wasp.py -- <glb> <out.json>
"""
import bpy
import json
import math
...
ROOT = HERE.parents[3]
DEFAULT_GLB = ROOT / 'docs/design/blender/wasp/wasp.glb'
DEFAULT_JSON = ROOT / 'docs/design/blender/wasp/validation.json'
...
ANCHORS = {  # authoring space: +Y forward, +Z up
    'muzzle': (0.000, 0.398, 0.020),
    'grip': (0.045, 0.060, -0.010),
    'sight': (0.000, -0.005, 0.112),
}
HEAT_BAND = (0.3145, 0.3905)
BORE_R = 0.0380
BORE_X, BORE_Z = 0.000, 0.020
...
for obj in meshes:
    ...
    for v in mesh.vertices:
        p = mw @ v.co
        tip_y = max(tip_y, p[1]) if abs(p[0] - BORE_X) < 0.05 and abs(p[2] - BORE_Z) < 0.05 else tip_y
...
check('muzzle_tip', abs(tip_y - 0.398) < 2e-3, f'forward-most bore vertex y={tip_y:.5f}')
check('heat_band_radius', abs(band_r - BORE_R) < 1e-3,
      f'max radius in band {band_r:.5f} over {band_n} verts (contract 0.0380)')
if band_n == 0:
...
report = {'asset': 'WASP', 'glb': str(GLB), 'passed': not failures,
