"""Write the browser-facing runtime glTF for WASP, plus .bin and manifest.

    blender --background docs/design/blender/wasp/wasp.blend \
        --python tools/blender/wasp/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `wasp.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space; any `round`-flagged meshes stay centred on their own origin as
      direct children of `extra` carrying their own translation (the SMG slot
      is `mag`-type and consumes no rounds, so none are expected)
...
that protected the live session also carried them into wasp.blend. Any
scene that is not the WASP study is removed here and the .blend is
...
UV_SCALE = 3.6  # texture tiles per metre; must match build-wasp.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz smg study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz smg study'
...
def material_desc(name, material):
    ...
    result = {'name': f'WASP | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
