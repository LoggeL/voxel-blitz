...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz rocket study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz rocket study'
...
# --- hinge contract ----------------------------------------------------------
# The gate bakes hinge-relative, so a drifted bore axis must fail here rather
# than ship a reload that swings around the wrong point.
muzzle_obj = next((o for o in scene.objects if o.name.split('.')[0] == 'muzzle'), None)
if muzzle_obj is None:
    raise SystemExit('TORCH scene has no muzzle marker')
muzzle_game = GAME @ muzzle_obj.location
if abs(muzzle_game[0] - 0.0) > 1e-6 or abs(muzzle_game[1] - HINGE[1]) > 1e-6 \
        or abs(muzzle_game[2] + 0.78) > 1e-6:
    raise SystemExit(f'muzzle marker drifted off the rocket contract: {list(muzzle_game)}')
...
gate_children = []
for material in sorted(gate_batches):
    batch = gate_batches[material]
    attributes, indices = build_mesh(batch)
    mesh_index = len(document['meshes'])
    name = f'gate | {material_order[material]}'
    ...
    document['nodes'].append({'name': name, 'mesh': mesh_index,
                              'translation': [round(float(v), 6) for v in HINGE],
                              'extras': {'blenderAsset': SLUG, 'role': 'breech gate'}})
    gate_children.append(node_index)
    draws += 1
    triangles += len(batch['indices']) // 3
