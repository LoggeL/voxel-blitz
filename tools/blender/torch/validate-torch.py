...
Loads docs/design/blender/torch/torch.glb into a brand-new empty
factory scene and measures it against the frozen rocket runtime contract:

    blender --background --factory-startup --python validate-torch.py -- \\
        [<torch.glb> <validation.json>]
...
ANCHORS = {  # authoring space: +Y forward, +Z up
    'muzzle': (0.000, 0.780, 0.075),
    'grip': (0.045, 0.080, -0.020),
    'support': (-0.060, 0.400, -0.030),
    'sight': (0.000, 0.340, 0.175),
}
...
for obj in meshes:
    ...
    for v in mesh.vertices:
        p = mw @ v.co
        tip_y = max(tip_y, p[1]) if abs(p[0]) < 0.07 and abs(p[2] - 0.075) < 0.07 else tip_y
        if HEAT_BAND[0] <= p[1] <= HEAT_BAND[1]:
            band_n += 1
            band_r = max(band_r, math.hypot(p[0], p[2] - 0.075))
measured['triangles'] = triangles
check('triangle_budget', 10000 <= triangles <= 20000, f'{triangles} tris (budget 10k-20k)')
check('muzzle_tip', abs(tip_y - 0.780) < 2e-3, f'forward-most bore vertex y={tip_y:.5f}')
...
report = {'asset': 'TORCH', 'glb': str(GLB), 'passed': not failures,
