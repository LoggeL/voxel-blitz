#!/usr/bin/env python3
"""Independently validate the FANG delivery by re-importing it fresh.

Loads docs/design/blender/fang/fang.glb into a brand-new empty
factory scene and measures it against the frozen revolver runtime contract
(same anchors WASP was held to):

    blender --background --factory-startup --python validate-fang.py -- \\
        [<fang.glb> <validation.json>]
...
ROOT = HERE.parents[3]
DEFAULT_GLB = ROOT / 'docs/design/blender/fang/fang.glb'
DEFAULT_JSON = ROOT / 'docs/design/blender/fang/validation.json'
...
ANCHORS = {  # authoring space: +Y forward, +Z up
    'muzzle': (0.000, 0.520, 0.040),
    'grip': (0.042, 0.055, -0.012),
    'sight': (0.000, 0.020, 0.105),
}
HEAT_BAND = (0.3688, 0.4984)
BORE_R = 0.0180
BORE_X, BORE_Z = 0.000, 0.040
...
for obj in meshes:
    ...
    for v in mesh.vertices:
        p = mw @ v.co
        tip_y = max(tip_y, p[1]) if abs(p[0] - BORE_X) < 0.03 and abs(p[2] - BORE_Z) < 0.03 else tip_y
...
check('muzzle_tip', abs(tip_y - 0.520) < 2e-3, f'forward-most bore vertex y={tip_y:.5f}')
check('heat_band_radius', abs(band_r - BORE_R) < 1e-3,
      f'max radius in band {band_r:.5f} over {band_n} verts (contract 0.0180)')
if band_n == 0:
...
report = {'asset': 'FANG', 'glb': str(GLB), 'passed': not failures,
