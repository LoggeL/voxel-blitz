"""Write the browser-facing runtime glTF for FANG, plus .bin and manifest.

    blender --background docs/design/blender/fang/fang.blend \
        --python tools/blender/fang/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `fang.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space; the revolver has no loose rounds (actions.js drives a single
      speedloader group, never the cartridges array)
    * one node per contract part, one primitive per material - except the two
      animated assemblies, which stay separable: `mag` carries one child node
      per bucket (cylinder / cases / ejector / crane) and `bolt` carries the
      `hammer` child node beside its static pin, so the builder can re-parent
      each bucket into its animated pivot
...
session may hold other studies in the same Blender file, so any scene that is
not the FANG study is removed here and the .blend is re-saved, so the
...
TEXTURES = ['ivory-armor', 'orange-painted-metal', 'petrol-ballistic-fabric',
            'worn-gunmetal', 'worn-rubber']
UV_SCALE = 3.6  # texture tiles per metre; must match build-fang.py
...
# Animated sub-assemblies, matched on the authored object name. Everything the
# reload choreography and the firing cycle move must survive as its own mesh;
# merging e.g. the crane into the drum would freeze the swing-out reload.
BUCKETS = {
    'mag': [
        (('Swing cylinder', 'Cylinder flute'), 'cylinder'),
        (('Chamber mouth', 'Chamber center'), 'cases'),
        (('Ejector rod',), 'ejector'),
        (('Crane arm', 'Crane pivot'), 'crane'),
    ],
    'bolt': [
        (('Spur hammer', 'Hammer serration'), 'hammer'),
    ],
}


def bucket_for(part, name):
    """Return the separable bucket for one authored mesh, or '' to merge."""
    for prefixes, bucket in BUCKETS.get(part, ()):
        if name.startswith(prefixes):
            return bucket
    if part == 'mag':
        # Every mesh on the swing-out assembly rides an animated pivot; an
        # unmatched mesh would freeze mid-reload, so fail loud.
        raise SystemExit(f'{name} in group mag matches no animated bucket')
    return ''




def node_for(part, bucket):
    return f'{part} {bucket}' if bucket else part


scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz revolver study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz revolver study'
...
for obj in sorted(scene.objects, key=lambda o: o.name):
    ...
    if obj.type != 'MESH' or part not in GROUPS:
        continue
    # The FANG study predates the per-asset material tag and carries the legacy
    # BALLISTA key; prefer the asset tag when a rebuilt study provides it.
    key = obj.get('fang_material') or obj.get('ballista_material')
    if key is None:
        raise SystemExit(f'{obj.name} has no material tag')
    ...
    node = node_for(part, bucket_for(part, obj.name))
    if obj.get('round'):
        ...
        target = batches[(node, material_index[key])]
        offset = GAME @ world.translation
        part_batches[part].append((node, material_index[key]))
...
def material_desc(name, material):
    ...
    result = {'name': f'FANG | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
...
def emit_node(node):
    """Write one node with one primitive per material; count its draws."""
    global draws, triangles
    primitives = []
    for (node_name, material) in sorted({(n, m) for (n, m) in part_batches[node_parent(node)]}):
        if node_name != node:
            continue
        batch = batches[(node_name, material)]
    ...
    if not primitives:
        raise SystemExit(f'node {node} has no geometry')
    return primitives
...
for part in GROUPS:
    if part == 'extra':
        continue
    nodes = sorted({n for (n, _m) in part_batches[part]})
    if not nodes:
        raise SystemExit(f'part {part} has no geometry')
    if len(nodes) == 1 and nodes[0] == part:
        add_node(part, emit_node(part), extras={'blenderAsset': SLUG})
    else:
        # Animated assembly: one child node per separable bucket so the
        # builder can re-parent each bucket into its own pivot. Static
        # leftovers (the hammer pin) stay on the parent node itself.
        parent_primitives = emit_node(part) if part in nodes else None
        children = []
        for node in nodes:
            if node == part:
                continue
            mesh_index = len(document['meshes'])
            document['meshes'].append({'name': node,
                                       'primitives': emit_node(node)})
            node_index = len(document['nodes'])
            document['nodes'].append({'name': node, 'mesh': mesh_index,
                                      'extras': {'blenderAsset': SLUG}})
            children.append(node_index)
        add_node(part, parent_primitives, extras={'blenderAsset': SLUG},
                 children=children)
