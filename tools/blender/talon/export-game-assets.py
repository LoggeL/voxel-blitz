"""Write the browser-facing runtime glTF for TALON, plus .bin and manifest.

    blender --background docs/design/blender/talon/talon.blend \
        --python tools/blender/talon/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `talon.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space; TALON carries no rounds, so `extra` (like `mag`, `bolt` and
      `trigger`) exports as an empty identity node - the knife slot never
      reloads, cycles or feeds cartridges
...
not the TALON study is removed here and the .blend is re-saved, so the
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz tanto knife study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz tanto knife study'
...
def material_desc(name, material):
    ...
    result = {'name': f'TALON | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
    strength = float(shader.inputs['Emission Strength'].default_value)
    if strength > 0:
        emission = shader.inputs['Emission Color'].default_value
        result['emissiveFactor'] = [min(1.0, float(v) * strength) for v in emission[:3]]
...
for part in GROUPS:
    ...
    if not primitives:
        # The knife slot never reloads, cycles or feeds: parts without authored
        # geometry still export as empty identity nodes so the loader returns
        # one group per contract part.
        add_node(part, None, extras={'blenderAsset': SLUG})
        continue
