#!/usr/bin/env python3
"""Independently validate the HALO delivery by re-importing it fresh.

Loads docs/design/blender/halo/halo.glb into a brand-new empty
factory scene and measures it against the frozen sniper runtime contract
(same anchors PEREGRINE was held to):

    blender --background --factory-startup --python validate-halo.py -- \\
        [<halo.glb> <validation.json>]
...
ROOT = HERE.parents[3]
DEFAULT_GLB = ROOT / 'docs/design/blender/halo/halo.glb'
DEFAULT_JSON = ROOT / 'docs/design/blender/halo/validation.json'
...
check('muzzle_tip', abs(tip_y - 0.710) < 2e-3, f'forward-most bore vertex y={tip_y:.5f}')
...
report = {'asset': 'HALO', 'glb': str(GLB), 'passed': not failures,
