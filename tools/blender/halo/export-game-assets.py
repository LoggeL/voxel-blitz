"""Write the browser-facing runtime glTF for HALO, plus .bin and manifest.

    blender --background docs/design/blender/halo/halo.blend \
        --python tools/blender/halo/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `halo.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space (`extra` stays an empty identity node: the longarc reload swaps the
      battery cell, so the choreography owns `extra` outright)
...
that protected the live session also carried them into halo.blend. Any
scene that is not the HALO study is removed here and the .blend is
...
UV_SCALE = 3.6  # texture tiles per metre; must match build-halo.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz coilgun study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz coilgun study'
...
def material_desc(name, material):
    ...
    result = {'name': f'HALO | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
