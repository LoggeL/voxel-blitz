"""Write the browser-facing runtime glTF for IFRIT, plus .bin and manifest.

    blender --background docs/design/blender/ifrit/ifrit.blend \
        --python tools/blender/ifrit/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `ifrit.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space; `extra` ships empty (the transverse fuel tank owns `mag` and
      detaches as a complete assembly, so no round nodes travel)
...
session can hold other studies in the same Blender file. Any scene that is
not the IFRIT study is removed here and the .blend is re-saved, so the
...
UV_SCALE = 3.6  # texture tiles per metre; must match build-ifrit.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz flamethrower study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz flamethrower study'
...
for obj in sorted(scene.objects, key=lambda o: o.name):
    ...
    if obj.type != 'MESH' or part not in GROUPS:
        continue
    # Authoring carried a copy-paste prop name over from the BALLISTA study, so
    # the live meshes store `ballista_material`; accept the slug prop too.
    key = obj.get('ifrit_material', obj.get('ballista_material'))
...
def material_desc(name, material):
    ...
    result = {'name': f'IFRIT | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
