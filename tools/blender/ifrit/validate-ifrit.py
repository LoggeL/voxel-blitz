#!/usr/bin/env python3
"""Independently validate the IFRIT delivery by re-importing it fresh.

Loads docs/design/blender/ifrit/ifrit.glb into a brand-new empty
factory scene and measures it against the frozen flamethrower runtime contract:

    blender --background --factory-startup --python validate-ifrit.py -- \\
        [<ifrit.glb> <validation.json>]
...
ROOT = HERE.parents[3]
DEFAULT_GLB = ROOT / 'docs/design/blender/ifrit/ifrit.glb'
DEFAULT_JSON = ROOT / 'docs/design/blender/ifrit/validation.json'
...
ANCHORS = {  # authoring space: +Y forward, +Z up
    'muzzle': (0.000, 0.655, 0.065),
    'grip': (0.045, 0.080, -0.020),
    'support': (-0.060, 0.400, -0.030),
    'sight': (0.000, 0.300, 0.158),
}
HEAT_BAND = (0.4375, 0.6333)
BORE_R = 0.0425
...
for obj in meshes:
    ...
    for v in mesh.vertices:
        p = mw @ v.co
        tip_y = max(tip_y, p[1]) if abs(p[0]) < 0.05 and abs(p[2] - 0.065) < 0.05 else tip_y
        if HEAT_BAND[0] <= p[1] <= HEAT_BAND[1]:
            band_n += 1
            band_r = max(band_r, math.hypot(p[0], p[2] - 0.065))
measured['triangles'] = triangles
check('triangle_budget', 10000 <= triangles <= 20000, f'{triangles} tris (budget 10k-20k)')
check('muzzle_tip', abs(tip_y - 0.655) < 2e-3, f'forward-most bore vertex y={tip_y:.5f}')
check('heat_band_radius', abs(band_r - BORE_R) < 1e-3,
      f'max radius in band {band_r:.5f} over {band_n} verts (contract 0.0425)')
if band_n == 0:
...
# IFRIT ships no loose rounds: the reload detaches the whole tank, so `extra`
# must be present and childless.
extra_kids = sorted(o.name for o in imported if o.parent is not None and o.parent.name == 'extra')
check('extra_empty', len(extra_kids) == 0, f'children of extra: {extra_kids or "none"}')
report = {'asset': 'IFRIT', 'glb': str(GLB), 'passed': not failures,
