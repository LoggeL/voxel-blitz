"""Write the browser-facing runtime glTF for PIKE, plus .bin and manifest.

    blender --background docs/design/blender/pike/pike.blend \
        --python tools/blender/pike/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `pike.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space; `extra` stays an empty identity node for the reload choreography
      to own (the lance reload swaps the lance cell, so no loose rounds ride
      along)
...
that protected the live session also carried them into pike.blend. Any
scene that is not the PIKE study is removed here and the .blend is
...
UV_SCALE = 3.6  # texture tiles per metre; must match build-pike.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz rail-lance study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz rail-lance study'
...
def material_desc(name, material):
    ...
    result = {'name': f'PIKE | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
    ...
    if strength > 0:
        color = shader.inputs['Emission Color'].default_value
        result['emissiveFactor'] = [min(1.0, round(v * strength, 4)) for v in color[:3]]
