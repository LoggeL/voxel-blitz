#!/usr/bin/env python3
"""Independently validate the PIKE delivery by re-importing it fresh.

Loads docs/design/blender/pike/pike.glb into a brand-new empty
factory scene and measures it against the frozen lance runtime contract:

    blender --background --factory-startup --python validate-pike.py -- \\
        [<pike.glb> <validation.json>]
...
ROOT = HERE.parents[3]
DEFAULT_GLB = ROOT / 'docs/design/blender/pike/pike.glb'
DEFAULT_JSON = ROOT / 'docs/design/blender/pike/validation.json'
...
ANCHORS = {  # authoring space: +Y forward, +Z up
    ...
    'grip': (0.045, 0.100, 0.015),
    'support': (-0.055, 0.420, 0.005),
    'sight': (0.000, 0.100, 0.155),
}
HEAT_BAND = (0.4470, 0.6780)
BORE_R = 0.0195
...
check('heat_band_radius', abs(band_r - BORE_R) < 1e-3,
      f'max radius in band {band_r:.5f} over {band_n} verts (contract 0.0195)')
if band_n == 0:
...
report = {'asset': 'PIKE', 'glb': str(GLB), 'passed': not failures,
