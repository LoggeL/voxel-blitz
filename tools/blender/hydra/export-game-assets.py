"""Write the browser-facing runtime glTF for HYDRA, plus .bin and manifest.

    blender --background docs/design/blender/hydra/hydra.blend \
        --python tools/blender/hydra/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `hydra.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space. The belt-fed slot carries no loose rounds: the belt links and
      drum live in `mag` and `extra` ships empty at identity, exactly like the
      runtime group before attachments mount
...
that protected the live session also carried them into hydra.blend. Any
scene that is not the HYDRA study is removed here and the .blend is
...
TEXTURES = ['ivory-armor', 'orange-painted-metal', 'tan-webbing',
            'worn-gunmetal', 'worn-rubber']
UV_SCALE = 3.6  # texture tiles per metre; must match build-hydra.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz minigun study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz minigun study'
...
def material_desc(name, material):
    ...
    result = {'name': f'HYDRA | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
