#!/usr/bin/env python3
"""Independently validate the HYDRA delivery by re-importing it fresh.

Loads docs/design/blender/hydra/hydra.glb into a brand-new empty
factory scene and measures it against the frozen sniper runtime contract
(same anchors PEREGRINE was held to):

    blender --background --factory-startup --python validate-hydra.py -- \\
        [<hydra.glb> <validation.json>]
...
ROOT = HERE.parents[3]
DEFAULT_GLB = ROOT / 'docs/design/blender/hydra/hydra.glb'
DEFAULT_JSON = ROOT / 'docs/design/blender/hydra/validation.json'
...
ANCHORS = {  # authoring space: +Y forward, +Z up
    'muzzle': (0.000, 0.880, 0.055),
    'grip': (0.050, 0.100, 0.000),
    'support': (-0.060, 0.470, -0.012),
    'sight': (0.000, 0.200, 0.155),
}
HEAT_BAND = (0.499, 0.8382)
BORE_R = 0.0290
...
check('muzzle_tip', abs(tip_y - 0.880) < 2e-3, f'forward-most bore vertex y={tip_y:.5f}')
...
report = {'asset': 'HYDRA', 'glb': str(GLB), 'passed': not failures,
