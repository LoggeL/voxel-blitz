"""Write the browser-facing runtime glTF for BISON, plus .bin and manifest.

    blender --background docs/design/blender/bison/bison.blend \
        --python tools/blender/bison/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `bison.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space, except under `extra`: the feed-cover leaves (custom prop `cover`)
      are world-baked child nodes the builder re-hangs on the authored hinge,
      and the belt lead (custom prop `belt_lead`) is an origin-centred node
      whose translation is its home on the feed tray (see
      `public/js/guns/models/bison.js`).
...
that protected the live session also carried them into bison.blend. Any
scene that is not the BISON study is removed here and the .blend is
...
TEXTURES = ['ivory-armor', 'orange-painted-metal', 'petrol-ballistic-fabric',
            'tan-webbing', 'worn-gunmetal', 'worn-rubber']
UV_SCALE = 3.6  # texture tiles per metre; must match build-bison.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz lmg study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz lmg study'
# With the other studies gone the contract names are free again: an authoring
# session that already owned 'muzzle' or 'bolt' left this build's marker or
# group empty as 'muzzle.001' / 'bolt.001'.
for name in MARKERS + GROUPS:
    owner = next((o for o in scene.objects if o.name.split('.')[0] == name), None)
...
rounds = []
cover_parts = []
COVER_OBJECTS = {'Feed tray cover', 'Feed cover latch'}
COVER_HINGE_NAMES = {'Feed cover hinge'}
...
for obj in sorted(scene.objects, key=lambda o: o.name):
    ...
    if obj.get('cover') or obj.name.split('.')[0] in COVER_OBJECTS:
        # Every feed-cover leaf (lid, ribs, latch, pawls) rides the reload
        # hinge as its own node: world-baked vertices, identity translation,
        # so the builder can re-hang it under the hinge pivot.
        target = {'vertices': [], 'indices': [], 'lookup': {}}
        offset = Vector((0.0, 0.0, 0.0))
        cover_parts.append({'name': obj.name, 'part': part, 'material': material_index[key],
                            'translation': GAME @ world.translation, 'batch': target})
    elif obj.get('round'):
        # An origin-centred node (the belt lead): geometry stays centred on its
        # own origin and the authored placement travels as the node translation.
        target = {'vertices': [], 'indices': [], 'lookup': {}}
        offset = Vector((0.0, 0.0, 0.0))
        rounds.append({'name': obj.name, 'part': part, 'material': material_index[key],
                       'translation': GAME @ world.translation, 'batch': target,
                       'role': 'belt lead' if obj.get('belt_lead') else 'stripper round'})
    else:
...
def material_desc(name, material):
    ...
    result = {'name': f'BISON | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
    ...
    if strength:
        ...
        result['emissiveFactor'] = [min(1, v * strength) for v in emission[:3]]
...
for entry in rounds:
    ...
    document['nodes'].append({'name': entry['name'], 'mesh': mesh_index,
                              ...
                              'extras': {'blenderAsset': SLUG, 'role': entry['role']}})
...
for entry in cover_parts:
    ...
    document['nodes'].append({'name': entry['name'], 'mesh': mesh_index,
                              'translation': [round(float(v), 6) for v in entry['translation']],
                              'extras': {'blenderAsset': SLUG, 'role': 'feed cover'}})
    round_children.append(node_index)
    draws += 1
    triangles += len(entry['batch']['indices']) // 3
    part_triangles[f'extra | {entry["name"]}'] = len(entry['batch']['indices']) // 3
hinge_marks = [o for o in scene.objects
               if o.name.split('.')[0] in COVER_HINGE_NAMES]
cover_hinge = None
if hinge_marks:
    mid = Vector((0.0, 0.0, 0.0))
    for mark in hinge_marks:
        mid += stored_world(mark).translation
    mid /= len(hinge_marks)
    cover_hinge = [round(float(v), 6) for v in GAME @ mid]
