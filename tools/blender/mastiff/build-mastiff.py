"""Author MASTIFF, a Mossberg-590-style pump shotgun prop for Voxel Blitz.
...
runs the same Blender Python underneath):
    ...
    blender --background --factory-startup --python build-mastiff.py

or through the live Blender MCP session (protocol 5) on 127.0.0.1:9876:

    from pathlib import Path
    p = Path('.../tools/blender/mastiff/build-mastiff.py')
...
has to reach it, and asserted at build time:
    ...
    muzzle   (0, 0.060, -0.668)   -> y = 0.668, x = 0, z = 0.060
    bore axis x = 0, y = 0.060    -> x = 0, z = 0.060, exposed radius 0.0200
    grip     (0.045, 0.010, -0.11)-> y = 0.110, x = 0.045, z = 0.010
    support  (-0.055, -0.030, -0.33) -> y = 0.330, x = -0.055, z = -0.030 on `pump`
    sight    ghost-ring axis 0.100 -> z = 0.100
    breech   shotgun -0.110       -> y = 0.110
    trigger  shotgun -0.145       -> y = 0.145, blade tip z = -0.058
    bolt home shotgun -0.050      -> y = 0.050, face in the ejection window
    pump rest (0, 0.038, -0.30)   -> (0, 0.300, 0.038), slide + action bars
    heat band -0.362 .. -0.530    -> y = 0.362 .. 0.530

Design intent: a Mossberg-590-style pump shotgun. A steel receiver carries a
bare mid-band barrel over a full-length magazine tube tied by a barrel band,
ghost-ring ears plus a winged bead post on the 0.100 sight line, a pistol-grip
buttstock with recoil pad, and a receiver side-saddle with three shells. The
animated top-level `pump` node (ribbed slide plus twin action bars) rides at
PUMP_REST under the support palm.
...
Outputs (owned by this task):
    docs/design/blender/mastiff/mastiff.blend   editable, textures packed
    docs/design/blender/mastiff/mastiff.glb     portable GLB, images embedded
    docs/design/blender/mastiff/manifest.json    machine-readable record
"""
import bmesh
import bpy
import json
...
DOCS = ROOT / 'docs/design/blender/mastiff'
INK = ROOT / 'public/assets/blender/textures'
DOCS.mkdir(parents=True, exist_ok=True)

ASSET = 'MASTIFF'
SCENE_NAME = f'{ASSET} | Voxel Blitz pump shotgun study'
...
MUZZLE = (0.000, 0.668, 0.060)     # game (0.000, 0.060, -0.668)
GRIP = (0.045, 0.110, 0.010)       # game (0.045, 0.010, -0.110)
SUPPORT = (-0.055, 0.330, -0.030)  # game (-0.055, -0.030, -0.330), rides `pump`
SIGHT = (0.000, 0.000, 0.100)      # ghost-ring axis, game y = 0.100
MARKERS = {'muzzle': MUZZLE, 'grip': GRIP, 'support': SUPPORT, 'sight': SIGHT}

BORE_X, BORE_Z = 0.000, 0.060   # bore axis
BORE_R = 0.0200                 # exposed bore radius (BARREL_R.shotgun minus clearance)
BORE_IN = 0.0110                # muzzle mouth recess visual radius
BORE_START = 0.080              # buried in the receiver
MAG_R = 0.0185                  # magazine tube radius
MAG_Z = 0.018                   # magazine tube axis height
MAG_SPAN = (0.100, 0.590)       # tube rear (buried) to tube mouth
HEAT_BAND = (0.362, 0.530)      # -BREACH_Z + heatLen * barrelLen
BREECH_Y = 0.110                # BREACH_Z.shotgun -0.11
SIGHT_Z = 0.100                 # ghost-ring axis height (ironSights height)
REAR_SIGHT_Y = -0.060           # ghost-ring ear plane, game z +0.060
FRONT_SIGHT_Y = 0.600           # bead post plane, game z -0.600
PUMP_REST = (0.000, 0.300, 0.038)  # game (0, 0.038, -0.30)
TRIGGER_Y, TRIGGER_TIP_Z = 0.145, -0.058   # TRIGGER_Z.shotgun -0.145
BOLT_HOME_Y = 0.050             # BOLT_HOME.shotgun -0.050
...
MATERIALS = [
    ('blued steel', 'worn-gunmetal', .62, .42, (.20, .21, .24)),
    ('polymer', 'worn-rubber', .05, .82, (.16, .16, .18)),
    ('orange paint', 'orange-painted-metal', .15, .48, (.95, .55, .12)),
    ('shell red', 'orange-painted-metal', .10, .55, (.72, .16, .12)),
    ('brass', 'orange-painted-metal', .75, .32, (.85, .66, .22)),
    ('rubber', 'worn-rubber', 0, .92, (.35, .35, .37)),
...
ROUND_OBJECTS = []  # side-saddle shells: never merged, each its own node
PART_GROUP = {}
for group in GROUPS:
...
def add_text(text, name, part, material, loc, rot, size, extrude=.0012):
    ...
    label['part'] = part
    label['mastiff_material'] = material
...
# ===========================================================================
# BODY: Mossberg-590-style twin-tube pump shotgun — steel receiver, bare
# mid-band barrel over a full-length magazine tube, ghost-ring ears plus a
# winged bead post on the 0.100 sight line, pistol-grip buttstock and a
# receiver side-saddle carrier. Every mating face is embedded 1+ mm or stands
# proud: never flush.
# ===========================================================================
# Monolithic steel receiver. The barrel breech, the magazine tube mouth and
# both pump action bars bury 5+ mm into it; every other cluster reaches it
# transitively through those joints.
add('Steel receiver', 'body', 'blued steel', boxv((.058, .230, .090)),
    loc=(0, 0.000, 0.010), bevel=.005)
add('Receiver roof plate', 'body', 'blued steel', boxv((.044, .200, .010)),
    loc=(0, 0.000, 0.056), bevel=.0015)
add('Ejection window', 'body', 'rubber', boxv((.004, .110, .045)),
    loc=(.0285, 0.045, 0.027))
add('Trigger pin', 'body', 'blued steel', tube(.004, .066, 10, axis='X'),
    loc=(0, -0.062, -0.010))
add('Receiver pin front', 'body', 'blued steel', tube(.004, .066, 10, axis='X'),
    loc=(0, 0.095, 0.005))
pair('Receiver side plate', 'body', 'blued steel', boxv((.003, .150, .050)),
     loc=(.0295, -0.010, 0.005), bevel=.001)
# Tang safety: the Mossberg signature control, hi-viz orange on the top tang.
add('Tang safety', 'body', 'orange paint', boxv((.014, .030, .008)),
    loc=(0, -0.095, 0.058), bevel=.0015)
add('Loading gate', 'body', 'blued steel', boxv((.030, .090, .008)),
    loc=(0, -0.020, -0.037), bevel=.0015)

# Pistol grip flowing rearward under the receiver toward the stock junction
# (the frozen grip marker stays wrapped low on the grip's front face).
add('Pistol grip', 'body', 'polymer', prism([
    (-0.060, -0.008), (0.130, 0.006), (0.100, -0.055), (0.055, -0.112),
    (0.015, -0.100), (0.022, -0.050)], .060), bevel=.008)
add('Grip cap', 'body', 'rubber', boxv((.062, .055, .010)),
    loc=(0, 0.035, -0.108), rot=(math.radians(-8), 0, 0), bevel=.003)

# Full buttstock, embedded 5 mm into the receiver rear face so the joint can
# never read as a flush coplanar seam.
add('Buttstock', 'body', 'polymer', prism([
    (-0.110, 0.022), (-0.180, 0.030), (-0.250, 0.050), (-0.350, 0.028),
    (-0.344, -0.068), (-0.200, -0.052), (-0.110, -0.018)], .056), bevel=.006)
add('Recoil pad', 'body', 'rubber', boxv((.058, .018, .120)),
    loc=(0, -0.350, -0.020), bevel=.004)
add('Cheek riser', 'body', 'rubber', boxv((.058, .100, .010)),
    loc=(0, -0.255, 0.048), bevel=.003)
add('Butt sling stud', 'body', 'blued steel', boxv((.010, .010, .012)),
    loc=(0, -0.300, -0.068), bevel=.001)
# Bare 12-gauge barrel: exactly 0.0200 across the whole heat band, buried at
# the breech, with a ringed muzzle and a recessed dark mouth. Nothing wider
# than the bore inside the band span.
BORE_OBJECT = add('Shotgun barrel', 'body', 'blued steel',
                  tube_stacked(0.0200, BORE_START, MUZZLE[1],
                               step=0.02, segments=28),
                  bevel=0)
add('Bore breech collar', 'body', 'blued steel', tube(.023, .012, 16),
    loc=(BORE_X, 0.116, BORE_Z))
add('Muzzle ring', 'body', 'blued steel', washer(.0220, .0185, .010, 20),
    loc=(BORE_X, 0.658, BORE_Z))
add('Target crown', 'body', 'blued steel', washer(.0195, .0135, .006, 24),
    loc=(BORE_X, 0.6645, BORE_Z), bevel=0)
add('Muzzle mouth', 'body', 'rubber', tube(.0145, .003, 16),
    loc=(BORE_X, 0.6657, BORE_Z), bevel=0)

# Full-length magazine tube under the barrel, twin-tube Mossberg profile. The
# rear end buries 15 mm into the receiver; the cap and hi-viz follower band
# ride on the `mag` animation node.
add('Magazine tube', 'body', 'blued steel', tube(MAG_R, 0.490, 20),
    loc=(0, 0.345, MAG_Z), bevel=0)
add('Magazine cap', 'mag', 'blued steel', tube(.0200, .018, 12),
    loc=(0, 0.596, MAG_Z))
add('Follower band', 'mag', 'orange paint', washer(.0205, .0170, .014, 16),
    loc=(0, 0.572, MAG_Z))
# Barrel band tying both tubes together, ahead of the heat band.
add('Barrel band', 'body', 'blued steel', boxv((.052, .014, .062)),
    loc=(0, 0.575, 0.040), bevel=.0015)

# Winged bead post: the post top lands exactly on the 0.100 sight line, the
# ivory bead dots its rear face, and the wings flank it below the line.
add('Front ramp', 'body', 'blued steel', prism([
    (0.585, 0.078), (0.615, 0.078), (0.610, 0.092), (0.590, 0.092)], .016),
    bevel=.001)
POST_OBJECT = add('Front post', 'body', 'blued steel', boxv((.008, .016, .024)),
                  loc=(0, FRONT_SIGHT_Y, 0.088), bevel=.001)
add('Front bead', 'body', 'ivory coating', tube(.0035, .006, 10),
    loc=(0, 0.5910, 0.0965), bevel=0)
pair('Front wing', 'body', 'blued steel', boxv((.006, .022, .016)),
     loc=(.012, 0.600, 0.084), bevel=.001)

# Ghost-ring rear sight: a true enclosed aperture on a tall base, the hole
# centred just under the 0.100 sight line so the bead sits high in the ring.
add('Rear sight base', 'body', 'blued steel', boxv((.038, .070, .028)),
    loc=(0, REAR_SIGHT_Y, 0.066), bevel=.0015)
pair('Ghost-ring ear', 'body', 'blued steel', boxv((.008, .020, .036)),
     loc=(.0145, REAR_SIGHT_Y, 0.096), bevel=.001)
add('Ghost-ring strap', 'body', 'blued steel', boxv((.044, .016, .008)),
    loc=(0, REAR_SIGHT_Y, 0.109), bevel=.001)
# Receiver side-saddle carrier on the left flank with three elastic loops.
add('Side-saddle carrier', 'body', 'polymer', boxv((.014, .190, .050)),
    loc=(-0.034, 0.000, 0.015), bevel=.0015)
for index, shell_y in enumerate((-0.053, 0.000, 0.053)):
    add(f'Saddle loop {index + 1}', 'body', 'rubber',
        solid(recty(shell_y - 0.017, shell_y + 0.017, -0.014, 0.044), .024, 'X',
              recty(shell_y - 0.009, shell_y + 0.009, -0.006, 0.036)), bevel=.001)

# Identity markings, modelled as geometry on the stock flanks and the
# receiver rear strip (clear of the plates and the ejection window).
add_text('MASTIFF', 'Marking MASTIFF left', 'body', 'ivory coating',
         (-0.0275, -0.230, 0.000), (math.pi / 2, 0, -math.pi / 2), .013)
add_text('MASTIFF', 'Marking MASTIFF right', 'body', 'ivory coating',
         (0.0275, -0.230, 0.000), (math.pi / 2, 0, math.pi / 2), .013)
add_text('M590', 'Marking M590 left', 'body', 'ivory coating',
         (-0.0285, -0.1055, 0.005), (math.pi / 2, 0, -math.pi / 2), .007)
add_text('M590', 'Marking M590 right', 'body', 'ivory coating',
         (0.0285, -0.1055, 0.005), (math.pi / 2, 0, math.pi / 2), .007)

# ===========================================================================
# MAGAZINE: tube-fed, so `mag` owns only the cap and follower band above.
# ===========================================================================

# ===========================================================================
# BOLT: breech face riding in the ejection window at BOLT_HOME
# ===========================================================================
BOLT_OBJECT = add('Breech bolt face', 'bolt', 'blued steel',
                  boxv((.006, .090, .030)), loc=(.0300, BOLT_HOME_Y, 0.030),
                  bevel=.001)
add('Extractor claw', 'bolt', 'blued steel', boxv((.005, .022, .010)),
    loc=(.0335, 0.078, 0.038), bevel=.001)

# ===========================================================================
# TRIGGER: blade plus squared guard on the dedicated animation group
# ===========================================================================
TRIGGER_OBJECT = add('Trigger blade', 'trigger', 'blued steel',
                     boxv((.010, .014, .028)), loc=(0, TRIGGER_Y, -0.0430),
                     rot=(math.radians(10), 0, 0), bevel=.002)
add('Trigger guard', 'trigger', 'blued steel',
    solid(recty(0.095, 0.195, -0.075, -0.028), .030, 'X',
          recty(0.105, 0.185, -0.065, -0.040)), bevel=.003)
# Trigger guard tang running back along the receiver belly toward the stock,
# visually tying the forward trigger group into the grip junction.
add('Trigger guard tang', 'trigger', 'blued steel', boxv((.026, .125, .009)),
    loc=(0, 0.0375, -0.0385), bevel=.0015)

# ===========================================================================
# PUMP: the animated top-level `pump` node. The walnut-free black slide wraps
# the magazine tube around PUMP_REST; the support palm rides this group. Both
# action bars bury into the receiver ahead and into the slide behind.
# ===========================================================================
SLIDE_OBJECT = add('Forend slide', 'pump', 'polymer', boxv((.076, .145, .053)),
                   loc=(0, 0.2825, 0.0125), bevel=.006)
for index in range(7):
    pair(f'Forend rib {index + 1}', 'pump', 'polymer',
         boxv((.007, .012, .044)),
         loc=(.0385, 0.225 + index * 0.018, 0.0125), bevel=.001)
pair('Action bar', 'pump', 'blued steel', boxv((.008, .130, .012)),
     loc=(.024, 0.155, 0.028), bevel=.001)

# ===========================================================================
# EXTRA: three side-saddle shells, direct children of `extra`.
# Each shell stands brass-up (TacStar-style) through its saddle loop; the
# authored placement is carried by the object transform alone.
# ===========================================================================
def _roll_upright(shape):
    """Roll a Y-built profile -90 deg about X: +Y (crimp) points down."""
    (verts, faces) = shape
    return ([(x, z, -y) for (x, y, z) in verts], faces)


SHELL_HULL = [_roll_upright(shape) for shape in [
    offset(tube(.0100, .036, 14), dy=-0.0010),   # hull   z -0.017 .. 0.019
    taper(.0100, .0050, 0.017, 0.023, 14),       # crimp  z -0.023 .. -0.017
]]
SHELL_BASE = [_roll_upright(shape) for shape in [
    offset(tube(.0102, .012, 14), dy=-0.0250),   # head   z  0.019 .. 0.031
    offset(washer(.0118, .0080, .0035, 14), dy=-0.0310),  # rim
]]
for index, shell_y in enumerate((-0.053, 0.000, 0.053)):
    add_local(f'Shell {index + 1}', 'extra', 'shell red', SHELL_HULL,
              (-0.047, shell_y, 0.015))
    add_local(f'Shell {index + 1} base', 'extra', 'brass', SHELL_BASE,
              (-0.047, shell_y, 0.015))
...
for (key, value) in {'asset_id': 'mastiff',
                     'asset_name': 'MASTIFF',
...
tip_plane = sorted({round(p[1], 9) for (p, n) in points if n == bore_object.name and p[1] > 0.60})
trigger_object = TRIGGER_OBJECT
trigger_low = min((trigger_object.matrix_world @ v.co)[2] for v in trigger_object.data.vertices)
bolt_object = BOLT_OBJECT
bolt_centre_y = (sum((bolt_object.matrix_world @ v.co)[1]
                     for v in bolt_object.data.vertices)
                 / len(bolt_object.data.vertices))
slide_object = SLIDE_OBJECT
slide_front = max((slide_object.matrix_world @ v.co)[1] for v in slide_object.data.vertices)
post_object = POST_OBJECT
post_top = max((post_object.matrix_world @ v.co)[2] for v in post_object.data.vertices)
ear_tops = []
for _name in ('Ghost-ring ear', 'Ghost-ring ear.001'):
    _obj = bpy.data.objects.get(_name)
    if _obj is not None:
        ear_tops.append(max((_obj.matrix_world @ v.co)[2] for v in _obj.data.vertices))
ear_top = max(ear_tops)
shell_min_x = 1e9
for _obj in ROUND_OBJECTS:
    for vertex in _obj.data.vertices:
        shell_min_x = min(shell_min_x, (_obj.matrix_world @ vertex.co)[0])

CONTRACT = [
    ('muzzle_forward_vertex_y', tip[1], MUZZLE[1], 1e-6),
    ('muzzle_marker', MUZZLE[1], 0.668, 1e-9),
    ...
    ('grip_marker_y', GRIP[1], 0.110, 1e-9),
    ('grip_marker_z', GRIP[2], 0.010, 1e-9),
    ('support_marker_x', SUPPORT[0], -0.055, 1e-9),
    ('support_marker_y', SUPPORT[1], 0.330, 1e-9),
    ('support_marker_z', SUPPORT[2], -0.030, 1e-9),
    ('sight_axis_z', SIGHT[2], SIGHT_Z, 1e-9),
    ('front_post_top_z', post_top, SIGHT_Z, 1e-3),
    ('ghost_ring_ear_top_z', ear_top, 0.114, 1e-3),
    ('rear_sight_plane_y', REAR_SIGHT_Y, -0.060, 1e-9),
    ('front_sight_plane_y', FRONT_SIGHT_Y, 0.600, 1e-9),
    ('trigger_blade_y', TRIGGER_Y, 0.145, 1e-9),
    ('trigger_blade_tip_z', trigger_low, TRIGGER_TIP_Z, 1e-5),
    ('bolt_home_y', bolt_centre_y, BOLT_HOME_Y, 1e-3),
    ('pump_slide_front_y', slide_front, 0.355, 1e-3),
    ('side_saddle_shell_reach_x', shell_min_x, None, None),
]
contract_failures = []
...
if shell_min_x >= -0.050:
    contract_failures.append(f'side-saddle shells do not reach outboard -x: min x {shell_min_x:.4f}')
...
print(f'bore axis measured ({bore_x:+.9f}, {bore_z:+.9f}); slide front y={slide_front:+.4f}')
print(f'sight line z={SIGHT_Z:.3f}: post top {post_top:.4f}, ear tops {ear_top:.4f}; '
      f'shells reach x={shell_min_x:+.4f}')
...
def normalize_glb(path):
    ...
    for node in document.get('nodes', []):
        ...
        if name in GROUPS:
            node.setdefault('extras', {})['blenderAsset'] = 'mastiff'
        if name:
            if name in seen:
...
manifest = {
    'asset': 'MASTIFF',
    'asset_id': 'mastiff',
    'kind': 'Mossberg-590-style pump shotgun prop (visual game asset)',
    'blender': bpy.app.version_string,
    'authoring_script': 'tools/blender/mastiff/build-mastiff.py',
    ...
    'contract_points': {'breech_y': BREECH_Y, 'bore_axis': [BORE_X, BORE_Z],
                        ...
                        'exposed_bore': [MAG_SPAN[1], MUZZLE[1]],
                        'heat_band': [-HEAT_BAND[1], -HEAT_BAND[0]],
                        'sight_height': SIGHT_Z,
                        'sight_planes': [-REAR_SIGHT_Y, -FRONT_SIGHT_Y],
                        'pump_rest': list(PUMP_REST)},
    'files': {'blend': 'docs/design/blender/mastiff/mastiff.blend',
              'glb': 'docs/design/blender/mastiff/mastiff.glb',
              'renders': ['render-hero.png', 'render-side.png', 'render-left.png',
                          'render-ads.png', 'render-rear.png'],
              'validation': 'docs/design/blender/mastiff/validation.json'},
    'build_time_checks': {
        'anchor_contract': 'muzzle plane, bore axis and exposed 0.0200 radius across '
                           'the whole heat band, all four markers, the 0.100 sight '
                           'line with post top and ghost-ring ear tops, the trigger '
                           'blade, the bolt home position, the pump slide front and '
                           'the side-saddle shell reach are asserted against the '
    ...
    'notes': ['Fresh design: authored from the weapon-brief shotgun row, not from another study.',
              'Twin-tube Mossberg-590 profile: bare mid-band barrel over a '
              'full-length magazine tube tied by a barrel band.',
              'The pump slide, action bars and support marker honour PUMP_REST; '
              'the slide front ends clear of the heat band.',
              'Markings are modelled geometry, not a texture decal.',
              'The side-saddle shells are hidden by the reload choreography; their '
