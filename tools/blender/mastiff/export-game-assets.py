"""Write the browser-facing runtime glTF for MASTIFF, plus .bin and manifest.

    blender --background docs/design/blender/mastiff/mastiff.blend \
        --python tools/blender/mastiff/export-game-assets.py
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `mastiff.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local
      space, except the side-saddle shell rounds, which are direct children of
      `extra` and carry their own translation (public/js/guns/actions.js writes
      position/rotation on the single reload shell each frame)
...
that protected the live session also carried them into mastiff.blend. Any
scene that is not the MASTIFF study is removed here and the .blend is
...
SLUG = 'mastiff'
SCENE_PREFIX = 'MASTIFF'

GROUPS = ['body', 'mag', 'bolt', 'pump', 'trigger', 'extra']
MARKERS = ['grip', 'muzzle', 'sight', 'support']
TEXTURES = ['ivory-armor', 'orange-painted-metal', 'worn-gunmetal', 'worn-rubber']
UV_SCALE = 3.6  # texture tiles per metre; must match build-mastiff.py
...
scene = next((s for s in bpy.data.scenes if s.name == f'{SCENE_PREFIX} | Voxel Blitz pump shotgun study'),
...
scene.name = f'{SCENE_PREFIX} | Voxel Blitz pump shotgun study'
...
def material_desc(name, material):
    ...
    result = {'name': f'MASTIFF | {name}', 'pbrMetallicRoughness': pbr, 'extras': extras}
...
for entry in rounds:
    ...
    document['nodes'].append({'name': entry['name'], 'mesh': mesh_index,
                              ...
                              'extras': {'blenderAsset': SLUG, 'role': 'round'}})
