"""Write the browser-facing runtime glTF for KESTREL revision 2, plus .bin and manifest.
...
game's loader (`public/js/engine/blender-assets.js`) expects.
    ...
    * buffer `kestrel.bin`, referenced as a sibling URI
    ...
    * node transforms are identity with all vertices baked into gun-local space
    * one node per contract part (`body`, `mag`, `bolt`, `trigger`,
      `factory-optic`, `extra`), one primitive per material
...
that protected the live session also carried them into kestrel.blend. Any
scene that is not the KESTREL study is removed here and the .blend is
...
SLUG = 'kestrel'
SCENE_PREFIX = 'KESTREL'
SCENE_NAME = f'{SCENE_PREFIX} | Voxel Blitz rifle study'
...
UV_SCALE = 3.6  # texture tiles per metre; must match build-kestrel.py
...
scene = next((s for s in bpy.data.scenes if s.name == SCENE_NAME), None) or \
    next((s for s in bpy.data.scenes if s.name.startswith(SCENE_PREFIX) and 'rifle' in s.name), None)
if scene is None:
    raise SystemExit(f'no {SCENE_PREFIX} revision 2 scene in {BLEND}')
...
for part in GROUPS:
    ...
    if not primitives and part != 'extra':
        raise SystemExit(f'part {part} has no geometry')
    # `extra` ships as an empty node: the rifle reload moves the magazine only.
    add_node(part, primitives or None, extras={'blenderAsset': SLUG})
for name in MARKERS:
...
# The study manifest records the runtime stage the way the r1 exporter did.
study_path = DOCS / 'manifest.json'
...
study['runtime'] = {'path': f'public/assets/blender/{SLUG}.gltf',
                    'exporter': 'tools/blender/kestrel/export-game-assets.py',
                    'slot': 'rifle', 'draws': draws, 'triangles': triangles,
