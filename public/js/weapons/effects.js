// Pooled gunshot, impact, and gore FX: tracers, flashes, voxel bursts,
// block shatter, brass, blood mist/droplets/stains, and optional trauma shake.
// Hard instance budgets and slot reuse keep steady-state updates allocation-free.
// Zero textures fetched — the one flash texture is a procedural canvas.
...
export class Effects {
  constructor(scene, camera, worldGetBlockFn) {
    ...
    // ---- gore: four fixed instanced pools, so bursts add no meshes/materials.
    // Opaque instance colors are faded by scale as Three's basic instancing has
    // no per-instance alpha without a custom shader.
    ...
    this.goreMeshes = [
      this.goreMistMesh, this.goreDropletMesh, this.goreStainMesh, this.goreVeilMesh,
    ];
    ...
    for (let i = 0; i < GORE_MIST_POOL_SIZE; i++) {
      this.goreMist[i] = {
        active: false, t: 0, life: 0, x: 0, y: 0, z: 0,
        vx: 0, vy: 0, vz: 0, size: 0,
      };
      this.hideInstance(this.goreMistMesh, i);
      this.goreMistMesh.setColorAt(i, this._col.setRGB(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b));
    }
    for (let i = 0; i < GORE_DROPLET_POOL_SIZE; i++) {
      ...
      this.hideInstance(this.goreDropletMesh, i);
      this.goreDropletMesh.setColorAt(i, this._col.setRGB(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b));
    }
    for (let i = 0; i < GORE_STAIN_POOL_SIZE; i++) {
      ...
      this.hideInstance(this.goreStainMesh, i);
      this.goreStainMesh.setColorAt(i, this._col.setRGB(BLOOD_DARK.r, BLOOD_DARK.g, BLOOD_DARK.b));
    }
    for (let i = 0; i < GORE_VEIL_POOL_SIZE; i++) {
      this.goreVeil[i] = {
        active: false, t: 0, life: 0, side: 0, lift: 0, size: 0, rot: 0,
      };
      this.hideInstance(this.goreVeilMesh, i);
      this.goreVeilMesh.setColorAt(i, this._col.setRGB(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b));
    }
  ...
  /**
   * Pooled blood response for a server-authored hit coordinate.
  ...
  gore(ev, { lethal = false, local = false } = {}) {
    ...
    const suppliedNormal = Number.isFinite(nx) && Number.isFinite(ny) && Number.isFinite(nz)
    ...
    for (let i = 0; i < mistCount; i++) {
      const idx = this.claimGoreSlot(this.goreMist);
    ...
    for (let i = 0; i < dropletCount; i++) {
      const idx = this.claimGoreSlot(this.goreDroplets);
    ...
    // Only attach an immediate decal when the event carries a genuine surface
    // normal. Other stains are created by droplets actually contacting voxels.
    if (suppliedNormal) {
    ...
    if (local && this.camera) {
      ...
      for (let i = 0; i < veilCount; i++) {
        const idx = this.claimGoreSlot(this.goreVeil);
        const v = this.goreVeil[idx];
        v.active = true; v.t = 0;
        v.life = (lethal ? 0.72 : 0.48) * (0.75 + Math.random() * 0.35);
        // Stay at the periphery: side never enters the central sight picture.
        v.side = (i & 1 ? 1 : -1) * (0.78 + Math.random() * 0.32);
        v.lift = (Math.random() - 0.5) * 1.35;
        v.size = (lethal ? 0.048 : 0.035) * (0.72 + Math.random() * 0.55);
        v.rot = Math.random() * TAU;
      }
    }
  ...
  claimGoreSlot(pool) {
    for (let i = 0; i < pool.length; i++) if (!pool[i].active) return i;
  ...
  spawnBloodStain(x, y, z, nx, ny, nz, size) {
    ...
    const idx = this.claimGoreSlot(this.goreStains);
    const stain = this.goreStains[idx];
    stain.active = true; stain.t = 0;
    stain.life = 2.1 + Math.random() * 1.2;
    stain.x = x; stain.y = y; stain.z = z;
    stain.size = size * (0.75 + Math.random() * 0.5);
    this._q.setFromUnitVectors(this._forward.set(0, 0, 1), this._normal);
    stain.qx = this._q.x; stain.qy = this._q.y; stain.qz = this._q.z; stain.qw = this._q.w;
    this._m4.compose(
      this._v.set(x, y, z), this._q,
  ...
  update(dt) {
    ...
    // blood mist expands and hangs briefly, distinct from the fast orange spark
    // cubes in the shared particle pool.
    for (let i = 0; i < this.goreMist.length; i++) {
      ...
      p.vx *= drag; p.vy = p.vy * drag - 2.2 * dt; p.vz *= drag;
      p.x += p.vx * dt; p.y += p.vy * dt; p.z += p.vz * dt;
      const fade = 1 - u;
      const sc = p.size * (0.7 + u * 3.2) * Math.min(1, fade * 3.5);
      this._s.set(sc * 1.45, sc, sc * 1.45);
    ...
    // Ballistic beads terminate on their first solid voxel contact and hand a
    // surface-oriented mark to the bounded stain pool.
    for (let i = 0; i < this.goreDroplets.length; i++) {
      ...
      this._s.set(p.size, p.size * (1.35 + Math.min(2.4, speed * 0.2)), p.size);
    ...
    for (let i = 0; i < this.goreStains.length; i++) {
      ...
      this._m4.compose(
        this._v.set(stain.x, stain.y, stain.z), this._q,
  ...
  dispose() {
    for (const mesh of [
      this.tracerMesh, this.partMesh, ...this.impactMeshes, ...this.goreMeshes,
    ]) {
