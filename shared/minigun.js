...
export const MINIGUN = Object.freeze({ spinUp: 0.7, spinDown: 0.45, heatPerShot: 0.0135,
...
export function stepMinigun(state, dt, held) {
  dt = Math.max(0, Number.isFinite(dt) ? dt : 0);
  const driving = held && !state.overheated;
  ...
  const coolingDt = driving ? Math.min(dt, (1 - state.spin) * MINIGUN.spinUp) : dt;
  ...
  return driving && !state.overheated && state.spin >= 1;
}
...
export function heatMinigun(state) {
  state.heat = Math.min(1, state.heat + MINIGUN.heatPerShot);
